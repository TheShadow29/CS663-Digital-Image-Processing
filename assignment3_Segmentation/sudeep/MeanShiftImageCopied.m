function [clustCent, data2cluster, outputimg] = MeanShiftImageCopied(input_image,bandWidth,plotFlag);
...
%*** Create set of dataPts ***%

h = fspecial('gaussian', 3, 1);
img1 = imfilter(input_image, h);
...
disp(size(data2cluster));
outmat = reshape(data2cluster, [256 256]);
outputimg = zeros(size(img1));
for i = 1:size(clustCent,2)
outputimg(:,:,1) = outputimg(:,:,1)+ double(outmat == i)*clustCent(1,i);
outputimg(:,:,2) =  outputimg(:,:,2)+double(outmat == i)*clustCent(2,i);
outputimg(:,:,3) =outputimg(:,:,3)+ double(outmat == i)*clustCent(3,i);
end
